...
DEFAULT_IMAGE_NAME="pclp1-cb-cd/2024-2025/tema-1-private"
DEFAULT_TAG='latest'
DEFAULT_REGISTRY='gitlab.cs.pub.ro'
...
checker_main() {
    ...
    docker run --rm \
            --mount type=bind,source="$tmpdir",target="$MOUNT_PROJECT_DIRECTORY" \
            "$image_name" /bin/bash -c "rm -rf /usr/local/bin/bash; cd \"$MOUNT_PROJECT_DIRECTORY\"; \"$MOUNT_PROJECT_DIRECTORY/checker/checker.py\" \"${script_args[@]}\"" # remove bash middleware script
